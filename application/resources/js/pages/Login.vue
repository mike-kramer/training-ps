<template>
  <GuestLayout subtitle="Sign in to manage cashboxes">
    ...
    <p class="mt-4 text-center text-sm text-slate-500">
      No account?
      <RouterLink to="/register" class="font-medium text-teal-700 hover:underline">Register</RouterLink>
    </p>
  </GuestLayout>
</template>
...
async function onSubmit() {
  ...
  try {
    await login({ ...form });
    router.push(route.query.redirect || { name: 'cashboxes' });
  } catch (error) {
    errors.value = validationErrors(error) || {};
    formError.value = errorMessage(error, 'Invalid credentials');
  } finally {
    loading.value = false;
  }
}
